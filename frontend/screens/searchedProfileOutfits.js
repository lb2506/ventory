...
const SearchedProfileOutfits = ({ route }) => {
  ...
  const fetchOutfits = async () => {
    try {
      const response = await axios.get(`${url}/user/outfits/${route.params.userId}`);
      setOutfits(response.data);
      setListOutfitsShowed(response.data);
      setIsFetched(true);
    } catch (error) {
      console.error(error);
    }
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => {
          navigation.navigate("SocialOutfitDetails", { item: item });
        }}
  ...
  return (
    <View style={styles.container}>
      ...
      {listOutfitsShowed.length === 0 && isFetched && <Text style={styles.noOutfitsText}>Aucun outfit.</Text>}
      <FlatList
